    ...
    DataMap &cplData)
{
  ...
  if (_firstIteration && (_firstTimeStep || _forceInitialRelaxation)) {
    ...
    if (not _firstIteration) {
      ...
      if (not math::equals(utils::MasterSlave::l2norm(_values), 0.0)  && utils::MasterSlave::l2norm(_values) < 1) {
        residualMagnitude /= utils::MasterSlave::l2norm(_values);
      }

      PRECICE_CHECK(not math::equals(residualMagnitude, 0.0), "Attempting to add a zero vector to the quasi-Newton V matrix. This means that the residual "
